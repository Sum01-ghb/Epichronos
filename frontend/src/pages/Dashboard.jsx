...
export default function Dashboard() {
  ...
  const stats = [
    { label: "Total Analyzed", value: loading ? "-" : totalAnalyzed, icon: Users, color: "text-blue-500", bg: "bg-blue-500/10", border: "border-blue-500/20" },
    { label: "High Risk", value: loading ? "-" : highRisk, icon: Activity, color: "text-rose-500", bg: "bg-rose-500/10", border: "border-rose-500/20" },
    { label: "Moderate Risk", value: loading ? "-" : modRisk, icon: AlertTriangle, color: "text-amber-500", bg: "bg-amber-500/10", border: "border-amber-500/20" },
    { label: "Low Risk", value: loading ? "-" : lowRisk, icon: ShieldCheck, color: "text-emerald-500", bg: "bg-emerald-500/10", border: "border-emerald-500/20" },
  ];
  ...
  const weeklyTrendData = useMemo(() => {
    ...
    return past7Days.map(dateStr => {
       const count = reports.filter(r => r.risk_level === "High" && r.analysis_date && r.analysis_date.startsWith(dateStr)).length;
  ...
  return (
    <div className={`min-h-screen relative overflow-hidden bg-linear-to-br from-slate-50 via-blue-50/60 to-teal-50/60 pb-32 transition-opacity duration-1000 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
      {/* Background Orbs */}
      ...
      <div className="relative mx-auto max-w-[1400px] px-4 pt-4 sm:px-6 lg:px-8 space-y-6 lg:space-y-8">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
            <div className="space-y-6 flex-1 text-center md:text-left">
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-white tracking-tight leading-tight">
                Advanced <span className="text-transparent bg-clip-text bg-linear-to-r from-sky-400 to-teal-300">cancer risk</span> analysis powered by intelligence.
              </h1>
              
              <p className="max-w-xl text-lg text-slate-300 font-medium leading-relaxed mx-auto md:mx-0">
                Prioritize at‑risk patients using multi‑modal risk scores, temporal trends, and explainable AI outputs tailored to your population.
              </p>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {stats.map((stat, i) => (
            <div key={i} className={`group relative overflow-hidden rounded-[2rem] bg-white/60 backdrop-blur-xl p-6 lg:p-8 border ${stat.border} shadow-xl shadow-slate-200/40 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl`}>
              <div className="flex items-center justify-between mb-4">
                 <div className={`p-3.5 rounded-2xl ${stat.bg} ${stat.color} transition-colors group-hover:bg-white group-hover:shadow-sm`}>
                    <stat.icon className="w-6 h-6" />
                 </div>
              </div>
              <div>
                 <p className="text-[11px] font-extrabold uppercase tracking-widest text-slate-500">{stat.label}</p>
                 <p className="mt-1 text-4xl font-black tracking-tight text-slate-800">
                   {loading ? <Loader2 className="w-6 h-6 animate-spin mt-2 text-slate-400" /> : stat.value}
                 </p>
              </div>
              <div className={`absolute bottom-0 left-0 w-full h-1.5 ${stat.bg}`} />
            </div>
          ))}
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="col-span-1 lg:col-span-1 border border-white/60 bg-white/60 backdrop-blur-xl rounded-[2.5rem] p-6 sm:p-8 flex flex-col shadow-xl shadow-slate-200/40 transition-all hover:-translate-y-1 hover:shadow-2xl group">
             <div className="flex items-center justify-between mb-6">
                <div>
                   <h3 className="text-lg font-bold text-slate-800 tracking-tight">Risk Distribution</h3>
                   <p className="text-xs text-slate-500 font-medium">Across all analyzed patients</p>
                </div>
                ...
                {loading ? (
                  ...
                  <>
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                       <span className="text-4xl font-black text-slate-800 tracking-tighter">{totalAnalyzed}</span>
          ...
          <div className="col-span-1 lg:col-span-1 border border-white/60 bg-white/60 backdrop-blur-xl rounded-[2.5rem] p-6 sm:p-8 flex flex-col shadow-xl shadow-slate-200/40 transition-all hover:-translate-y-1 hover:shadow-2xl group">
             <div className="flex items-center justify-between mb-8">
                <div>
                   <h3 className="text-lg font-bold text-slate-800 tracking-tight">High-Risk Trend</h3>
                   <p className="text-xs text-slate-500 font-medium">Cases flagged last 7 days</p>
                </div>
             ...
             <div className="flex-1 min-h-[220px] w-[calc(100%+20px)] -ml-5">
                {loading ? (
                   ...
                   <div className="flex items-center justify-center h-full w-full">
                     <p className="text-sm text-slate-500 font-medium ml-4">No high-risk cases detected recently.</p>
                   </div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={weeklyTrendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      ...
                      />
                      <Line type="monotone" dataKey="risk" stroke="#f43f5e" strokeWidth={4} dot={{ r: 5, strokeWidth: 2, fill: '#fff' }} activeDot={{ r: 7, strokeWidth: 0, fill: '#f43f5e' }} />
                    </LineChart>
                  </ResponsiveContainer>
                )}
             </div>
          ...
          <div className="col-span-1 lg:col-span-1 flex flex-col gap-6">
             ...
             <div className="flex-1 border border-white/60 bg-white/60 backdrop-blur-xl rounded-[2.5rem] p-6 lg:p-8 shadow-xl shadow-slate-200/40 transition-all hover:-translate-y-1 hover:shadow-2xl">
                <div className="flex items-center gap-4 mb-6">
                   ...
                   <div>
                     <h3 className="text-lg font-bold text-slate-800 tracking-tight">Recent Analyses</h3>
                     <p className="text-xs font-semibold text-slate-500">Latest predictive reports</p>
                   </div>
                </div>
                <div className="space-y-3.5">
                   {loading ? (
                     ...
                     recentPatients.map((rep, idx) => (
                        <div key={idx} onClick={() => navigate('/patients')} className="group flex items-center justify-between p-3.5 rounded-2xl bg-white/80 border border-slate-100 shadow-sm hover:shadow-md hover:bg-white transition-all cursor-pointer">
                           <div className="flex flex-col">
                             <span className="font-bold text-slate-700 text-sm">{rep.patient_name || 'Unknown'}</span>
                             <span className="text-[10px] text-slate-400 font-semibold">{new Date(rep.analysis_date).toLocaleDateString()}</span>
                           </div>
                           <span className={`flex items-center gap-1.5 text-[11px] font-black tracking-wider ${
                               rep.risk_level === 'High' ? 'text-rose-600 bg-rose-100' : 
                               rep.risk_level === 'Moderate' ? 'text-amber-600 bg-amber-100' : 
                     ...
                     <p className="text-sm text-slate-500 text-center py-4">No patients analyzed yet.</p>
                   )}
                </div>
             </div>
             ...
             <div className="grid grid-cols-2 gap-4 h-28">
                <button onClick={() => navigate('/analyze')} className="group flex flex-col items-center justify-center gap-2 rounded-[2rem] bg-linear-to-br from-white to-sky-50 border border-sky-100 shadow-md hover:shadow-sky-500/20 hover:-translate-y-1 transition-all text-sky-700 cursor-pointer">
                   <Activity className="w-6 h-6 transition-transform group-hover:scale-110" />
                   <span className="text-xs font-bold tracking-tight">Analyze</span>
                </button>
                <button onClick={() => navigate('/patients')} className="group flex flex-col items-center justify-center gap-2 rounded-[2rem] bg-white border border-slate-200/60 shadow-md hover:shadow-lg hover:-translate-y-1 transition-all text-slate-600 cursor-pointer">
