  ...
  return (
    <header className="fixed inset-x-0 top-0 z-20 border-b border-sky-100/50 bg-linear-to-r from-sky-50/90 via-blue-50/80 to-teal-50/90 backdrop-blur-2xl shadow-sm shadow-sky-900/5 transition-all duration-300">
      <div className="relative mx-auto flex h-16 sm:h-20 items-center justify-between px-4 sm:px-6 lg:px-10">
        ...
        {/* Spacer to balance title alignment on desktop if needed, though absolute positioning handles it */}

        {/* Right section: Profile & Output */}
        <div className="flexItems-center gap-3 sm:gap-4 z-10 flex">
          {/* Profile initial */}
          <div className="flex h-10 w-10 sm:h-11 sm:w-11 cursor-default items-center justify-center rounded-full bg-linear-to-br from-sky-500 to-teal-400 text-sm font-extrabold text-white shadow-md ring-[3px] ring-white/80 shadow-sky-500/20 group hover:ring-sky-200 transition-all duration-300" title={displayName}>
            <span className="transition-transform duration-300 group-hover:scale-110">{userInitial}</span>
          </div>
