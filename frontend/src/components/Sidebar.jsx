        ...
        <nav className="relative flex h-full flex-col gap-3">
          {navItems.map(({ label, path, icon: Icon }) => (
            ...
            >
              {({ isActive }) => (
                <>
                  <Icon className={`h-5 w-5 transition-transform duration-300 ${isActive ? 'scale-110 text-white' : 'text-slate-400 group-hover:text-sky-500'}`} />
            ...
            <nav className="relative flex h-full flex-col gap-3">
              {navItems.map(({ label, path, icon: Icon }) => (
                ...
                >
                  {({ isActive }) => (
                    <>
                      <Icon className={`h-[22px] w-[22px] transition-transform duration-300 ${isActive ? 'scale-110 text-white' : 'text-slate-400 group-hover:text-sky-500'}`} />
